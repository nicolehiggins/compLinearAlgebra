...
# right rectangular rule function
right_rectangular_rule <- function(f, a, b, n) {
  h <- (b - a) / n  # interval size
  x <- seq(a + h, b, length.out = n)  # x vals
  y <- f(x)
  s <- sum(y) * h
  return(s)
}

# estimating area under fun using the right rectangular rule
area <- right_rectangular_rule(fun, -1, 1, 4)
print(area)
```
...
**another way to connect evenly spaced points is to use the second barycentric formula with rational (not polynomial) interpolation! Write some code to calculate**
...
**one problem with the Vandermonde method is that it can be numerically unstable for large n. It can result in a polynomial that oscillates widely between points, even when the function being interpolated is smooth.**

**further, the matrix you constructed contains n+1 columns and n+1 points are being used for interpolation. instead, construct the matrix as follows:**
```{r}
n <- 50
...
A <- outer(x, 0:(n-1), `^`)
```
**so that the number of points used for interpolation matches the number of columns in the Vandermonde matrix.**
...
x14 <- x^(14)
x16 <- x^(16)
plot(x, x14, type = 'l', col = 'purple')
points(x, x16, col = 'blue', type = 'l')
legend('topright', inset = 0.1, c("x^14", "x^16"), lty = 2, col = c("purple", "blue"))

T_14 = cos(14*acos(x))
T_16 = cos(16*acos(x))
plot(x, T_14, type = 'l', col = 'purple')
points(x, T_16, type = 'l', col = 'blue')
legend('topright', inset = 0.1, c("T_14", "T_16"), lty = 2, col = c("purple", "blue"))
```
**These two figures differ in their endpoint activity. While the exponential polynomials' y-values shoot up to infinity, the Chebyshev polynomials' y-values oscillate around the x axis; resulting in n extrema. When using either of these polynomials to approximate a function, the former may be imprecise at the end points, while the latter retains high precision across the domain. As such, Chebyshev approximations can lead to more efficient algorithms for numerical computation.**
